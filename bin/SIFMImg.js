...
(function() {
  var AWS, SIFMImg, fs, gm, imageMagick, request, s3, uuid;
  ...
  SIFMImg = (function() {
    ...
    SIFMImg.prototype.catbw = function(cb) {
      ...
      file.on('close', function() {
        ...
        return imageMagick(rs, _imgNum + ".jpeg").type("Grayscale").write(localImg, function(err) {
          if (err) {
            console.log(err);
          }
    ...
    SIFMImg.prototype.catsize = function(width, height, cb) {
      ...
      return request(img, function(err, resp, body) {
        var rs;
        rs = fs.createReadStream(localImg);
        return imageMagick(rs, this.imgNum + ".jpeg").resize(width, height).write(localImg, function(err) {
          if (err) {
            console.log(err);
          }
          return cb(localImg);
    ...
    SIFMImg.prototype.catsizebw = function(width, height, cb) {
      ...
      return request(img, function(err, resp, body) {
        var rs;
        rs = fs.createReadStream(localImg);
        return imageMagick(rs, this.imgNum + ".jpeg").resize(width, height).type("Grayscale").write(localImg, function(err) {
          if (err) {
            console.log(err);
          }
          return cb(localImg);
